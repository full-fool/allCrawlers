...
def getListFromFile(fileName):
    namelist = []
    urlList = []
    contentList = open(fileName).split('\n')
    for eachLine in contentList:
        namelist.append(eachLine.split(',')[0], eachLine.split(',')[1])
        if not eachLine.split(',')[1] in urlList:
            urlList.append(eachLine.split(',')[1])
    return namelist, len(urlList)
...
result, length = getListFromFile('typelist.txt')
print result
print length
sys.exit()
