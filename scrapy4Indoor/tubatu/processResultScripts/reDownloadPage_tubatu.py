...
def getReOpenPage():
    try:
        fileContent = open('log.txt').read().split('\n')
    except Exception as ep:
    ...
    for eachLine in fileContent:
        if eachLine == '\n' or eachLine == '\r' or eachLine == '':
            continue
        space = eachLine.split(',')[1]
        part = eachLine.split(',')[2]
        style = eachLine.split(',')[3]
        color = eachLine.split(',')[4]
        pageUrl = eachLine.split(',')[5]
        pageList.append((space, part, style, color, pageUrl))
    return pageList
...
for i in range(len(pageList)):
    ...
    style = pageList[i][2]
    color = pageList[i][3]
    pageUrl = pageList[i][4]
    
    findThread = False
    while findThread == False:
        for threadIter in range(threadNum):
            if not threadNumPool.has_key(threadIter):
                threadNumPool[threadIter] = ProcessOnePage(space, part, style, color, pageUrl)
