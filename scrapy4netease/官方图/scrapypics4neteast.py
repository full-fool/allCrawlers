...
def downloadPicsFromOnePage(carBrand,carType,url):
    ...
    outPicsNumPattern = re.compile(r'<a target="_self" href=".+?">官方\((\d+)\)</a>')
    ...
    #print 'outPicsNum is %s' % outPicsNum
    ...
    for i in range(pageNum):
        ...
        for picsSection in picsSectionList:
            ...
            if len(yearPattern.findall(carName.encode('utf8'))) != 0:
                ...
                twoDigitYearPattern = re.compile(r'([\d]{2})款')
                if len(twoDigitYearPattern.findall(carName.encode('utf8'))) == 0:
                    continue
                twoDigitYear = twoDigitYearPattern.findall(carName.encode('utf8'))[0]
                if int(twoDigitYear) >=0 and int(twoDigitYear) <=15:
                    year = '20'+ twoDigitYear
                elif int(twoDigitYear) >=90 and int(twoDigitYear) <= 99:
...
#downloadPicsFromOnePage('阿斯顿马丁', 'DB9', 'http://pic.auto.163.com/autopic/ckindex/topicid=293H0008,setid=0.html')
