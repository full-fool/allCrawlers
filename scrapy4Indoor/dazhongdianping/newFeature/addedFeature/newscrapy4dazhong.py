...
#getMatch('.',urllib.quote('/shop/21261679/dish-秘制嫩牛肉'))
#sys.exit()
...
def fetchAllInfoForOneShop(city, foodType, url):
    ...
    try:
        ...
        print ep.message
    filehandler.close()
    robustPrint('done rating for %s' % shopName.decode('utf8'))


    print 'url %s' % url
#<div class="shop-tab-recommend J-panel Hide">
#<script type="text/panel" class="J-panels">
    recommendScriptSection = pagesoup.find_all("script", attrs={"class": "J-panels"})[0]
    #recommendSection = recommendScriptSection.find_all("p", attrs={"class": "recommend-name"})[0]
                        #pagesoup.find_all("div", attrs={"class": "breadcrumb"})[0]
