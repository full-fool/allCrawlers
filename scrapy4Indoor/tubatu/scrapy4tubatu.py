    ...
    for j in range(jStartPoint, len(partList)):
        ...
        for k in range(kStartPoint, len(styleList)):
            ...
            for z in range(zStartPoint, len(colorList)):
                ...
                pageContent = getPageWithSpecTimes(0, pageUrl)
                if pageContent == None:
                    writeToLog('cannot open page,%s,%s,%s,%s' %(spaceList[i][1].decode('utf8'), partList[j][1].decode('utf8'),\
                        styleList[k][1].decode('utf8'), colorList[z][1].decode('utf8')))
                    setProcess('%s,%s,%s,%s' % (i,j,k,z))
                    continue
                #print pageContent
                #有两种图片的html格式
                ...
                while '下一页' in pageContent:
                    currentPageNum += 1
                    pageUrl = 'http://xiaoguotu.to8to.com/list.php?a1=0&a2=1&a3=&a4=%s&a5=%s&a6=%s&a7=%s&a8=&a9=0&p=%s' % \
                    (spaceList[i][0], partList[j][0], styleList[k][0], colorList[z][0], currentPageNum)
                    #pageUrl = 'http://xiaoguotu.to8to.com/list.php?a1=0&a2=1&a3=&a4=1&a5=336&a6=13&a7=1&a8=&a9=0&p=%s' % currentPageNum
                    pageContent = getPageWithSpecTimes(0, pageUrl)
                    if pageContent == None:
                        writeToLog('cannot open page for page,%s,%s,%s,%s,%s' %(spaceList[i][1].decode('utf8'), partList[j][1].decode('utf8'),\
                            styleList[k][1].decode('utf8'), colorList[z][1].decode('utf8'), currentPage))
                        break
                    picInfoList += picPattern1.findall(pageContent)
                    picInfoList += picPattern2.findall(pageContent)
                if len(picInfoList) == 0:
                ...
                dirName = os.path.join(spaceList[i][1].decode('utf8'), partList[j][1].decode('utf8'), styleList[k][1].decode('utf8'), colorList[z][1].decode('utf8'))
